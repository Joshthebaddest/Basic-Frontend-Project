...
function AddTask() {
    ...
    const handleClick = (e) => {
      if(TaskData.length > 1){
        taskDispatch({ type: 'ADDING_TASK', payload: { name: e.target.textContent  } })
      }
    }
    ...
    <>
        <div className='border-2 border-solid text-center'>
            <ul className='border-2 border-solid'>
              {TaskData.map(item => (
                <li key={item.id} className='py-2 border-b-2 border-solid cursor-pointer' onClick={handleClick}>{item.name} 
